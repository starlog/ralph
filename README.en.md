...
| `--model <name>` | Model: `sonnet` or `opus`. Defaults are per-command — `--plan` defaults to `opus` (reasoning-heavy); `--run` / `--task` / `--dry-run` / `--interactive` default to `sonnet` (throughput). The selected model is printed at the start of each run. |
