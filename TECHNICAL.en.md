...
|---|---|
| `tasks.json` has at least one `done: true` task (after `--run`) | post-plan snapshot — undoes the `--run`, leaves the plan |
| `tasks.json` exists but no task is done (after `--plan`) | pre-plan snapshot — pre-ralph state |
...
- **Rollback snapshots** — `--plan` saves pre/post snapshots automatically; `--rollback` lets you undo the last plan/run cycle wholesale.
